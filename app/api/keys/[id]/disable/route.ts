...
export async function POST(
  ...
  try {
    ...
    if (!keyRow.active) {
      return NextResponse.json(
        { error: { code: 'CONFLICT', message: 'Key is already disabled' } },
        { status: 409 }
      );
    }
    ...
    const result = await query<ApiKeyRow>(
      `UPDATE api_keys SET active = false, updated_at = NOW() WHERE id = $1 RETURNING *`,
      [keyId]
    );

    if (!result.rows[0]) {
      return NextResponse.json(
        { error: { code: 'DATABASE_ERROR', message: 'Failed to disable key' } },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { success: true, message: 'Key disabled' },
      { status: 200 }
    );
  } catch (error) {
